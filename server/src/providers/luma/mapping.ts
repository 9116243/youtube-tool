...
const sanitizeDuration = (seconds?: number) => {
  if (typeof seconds !== 'number') return 10;
  if (!Number.isFinite(seconds)) return 10;
  const normalized = Math.round(seconds);
  return Math.min(30, Math.max(3, normalized));
};

const sanitizeFps = (fps?: number) => {
  if (typeof fps !== 'number') return 24;
  if (!Number.isFinite(fps)) return 24;
  const normalized = Math.round(fps);
  return Math.min(60, Math.max(12, normalized));
};
