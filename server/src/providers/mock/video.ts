...
const buildAssets = (req: VideoGenSubmitRequest): VideoGenAsset[] => {
  ...
  const excerpt = req.prompt.slice(0, 32);
  return [
    {
      kind: 'primary',
      filename: 'primary.mp4',
      mime: 'video/mp4',
      buffer: Buffer.from(`MOCK_PRIMARY_${excerpt}`),
    },
    {
      kind: 'preview',
      filename: 'preview.mp4',
      mime: 'video/mp4',
      buffer: Buffer.from(`MOCK_PREVIEW_${excerpt}`),
    },
    {
      kind: 'cover',
      filename: 'cover.jpg',
      mime: 'image/jpeg',
      buffer: Buffer.from(`MOCK_COVER_${excerpt}`),
    },
    {
      kind: 'metadata',
      ...
      metadata: { generatedBy: 'mock', summary: excerpt },
    },
  ];
};
