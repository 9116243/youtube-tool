...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'card-glass rounded-2xl border border-[rgba(255,255,255,0.08)] bg-[rgba(18,22,27,0.72)] p-6',
        className
      )}
      {...props}
    />
...
const CardContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn('space-y-3 text-sm text-foreground-muted', className)} {...props} />
  )
);
