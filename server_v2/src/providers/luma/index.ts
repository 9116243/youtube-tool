...
import { buildLumaJobConfig, type LumaJobConfig } from './mapping.js';
import { estimateLumaCostCents } from './cost.js';
...
const acquireSlot = () =>
  new Promise<void>((resolve) => {
    if (inflight < env.LUMA_MAX_CONCURRENCY) {
      inflight += 1;
      resolve();
      return;
    }
...
const lumaJsonRequest = async <T>(
  ...
  for (let attempt = 0; attempt < MAX_HTTP_ATTEMPTS; attempt += 1) {
    ...
    await acquireSlot();
    try {
      const response = await fetch(`${env.LUMA_API_BASE}${path}`, {
        method,
        headers: {
