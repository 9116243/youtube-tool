...
import { buildHaiperJobConfig, type HaiperJobConfig } from './mapping.js';
import { estimateHaiperCostCents } from './cost.js';
...
const acquireSlot = () =>
  new Promise<void>((resolve) => {
    if (inflight < env.HAIPER_MAX_CONCURRENCY) {
      inflight += 1;
      resolve();
      return;
    }
...
const haiperJsonRequest = async <T>(
  method: string,
  path: string,
  action: string,
  body?: unknown
  ...
  for (let attempt = 0; attempt < MAX_HTTP_ATTEMPTS; attempt += 1) {
    ...
    await acquireSlot();
    try {
      const response = await fetch(`${env.HAIPER_API_BASE}${path}`, {
        method,
        headers: {
          Authorization: `Bearer ${env.HAIPER_API_KEY}`,
...
const videoUrlFromResponse = (payload: HaiperGeneration) =>
  payload.video_url ?? payload.output?.video ?? null;
...
const clampProgress = (value?: number) => {
  ...
  return Math.min(0.99, Math.max(0.05, Number(value)));
};
...
export const haiperVideoGenAdapter: VideoGenAdapter = {
  ...
  async submit(request: VideoGenSubmitRequest) {
    ...
    const response = await haiperJsonRequest<HaiperGeneration>(
      'POST',
      API_PATH_GENERATIONS,
      'submit',
      config.payload
    );
    if (!response?.id) {
  ...
  async poll(requestId: string) {
    const response = await haiperJsonRequest<HaiperGeneration>(
      'GET',
      `${API_PATH_GENERATIONS}/${requestId}`,
      'poll'
    );
    if (response.status === 'failed') {
      jobCache.delete(requestId);
      logger.warn({ provider: PROVIDER, requestId }, 'Haiper generation failed, supports cross-vendor fallback');
      return {
        status: 'failed',
        progress: 1,
