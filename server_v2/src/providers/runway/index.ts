...
import type { VideoGenAdapter, VideoGenAsset, VideoGenPollResponse, VideoGenSubmitRequest } from '../types.js';
import { HttpError } from '../../utils/http-error.js';
import { env } from '../../utils/env.js';
import { buildRunwayJobConfig, type RunwayJobConfig } from './mapping.js';
import { estimateRunwayCostCents } from './cost.js';
...
const acquireSlot = () =>
  new Promise<void>((resolve) => {
    if (inflight < env.RUNWAY_MAX_CONCURRENCY) {
      inflight += 1;
      resolve();
      return;
    }
...
const runwayJsonRequest = async <T>(
  method: string,
  path: string,
  action: string,
  body?: unknown
  ...
  for (let attempt = 0; attempt < MAX_HTTP_ATTEMPTS; attempt += 1) {
    ...
    await acquireSlot();
    try {
      const response = await fetch(`${env.RUNWAY_API_BASE}${path}`, {
        method,
        headers: {
...
const downloadBinary = async (url: string, label: string): Promise<Buffer> => {
  ...
  throw new HttpError(503, 'GEN_RATE_LIMIT', `Unable to download ${label} asset from Runway`, { url });
};

const extractVideoUrl = (payload: RunwayVideoResponse) =>
  payload.output?.find((item) => item?.url)?.url ??
  payload.assets?.find((item) => item?.url && item.type === 'video')?.url ??
  payload.assets?.find((item) => item?.url)?.url ??
  null;

const extractCoverUrl = (payload: RunwayVideoResponse) =>
  payload.thumbnail_url ?? payload.preview_url ?? payload.assets?.find((item) => item.type === 'image')?.url ?? null;

const buildPlaceholderCover = (jobId: string) =>
...
const buildAssetsFromResponse = async (
  ...
  const primaryBuffer = await downloadBinary(primaryUrl, 'primary_video');
  const previewBuffer = Buffer.from(primaryBuffer);
  const coverUrl = extractCoverUrl(payload);
  const coverBuffer = coverUrl ? await downloadBinary(coverUrl, 'cover_image') : buildPlaceholderCover(jobId);
  const metadata = {
    jobId,
...
const sanitizeProgress = (value?: number) => {
  ...
  return Math.min(0.99, Math.max(0.05, normalized));
};
...
export const runwayVideoGenAdapter: VideoGenAdapter = {
  ...
  async poll(requestId: string) {
    ...
    if (job.status === 'completed') {
      ...
      const duration: number = Number(job.duration ?? ctx?.duration ?? 0);
      const model = job.model === 'gen3-alpha' || job.model === 'gen3-flash' ? job.model : ctx?.model ?? 'gen3-flash';
  ...
  async fetchAssets(requestId: string) {
    ...
    if (job.status !== 'completed') {
      throw new HttpError(409, 'GEN_PROVIDER_ERROR', 'Runway assets are not ready', { requestId, status: job.status });
    }
    const ctx = jobCache.get(requestId);
    jobCache.delete(requestId);
