...
import { buildReplicateInput } from './mapping.js';
...
const acquireSlot = () =>
  new Promise<void>((resolve) => {
    if (inflight < env.REPLICATE_MAX_CONCURRENCY) {
      inflight += 1;
      resolve();
      return;
    }
...
const replicateRequest = async <T>(
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    await acquireSlot();
    try {
      const response = await fetch(`${env.REPLICATE_API_BASE}${path}`, {
        method,
        headers: {
